...
class Api::V1::TripsController < ApplicationController
  ...
  def create
    ...
    if Location.all.find {|location| location[:name_suffix].split(",")[0].gsub(/[^a-zA-Z]/, '').downcase.include? (params[:trip][:trip_title].gsub(/[^a-zA-Z]/, '').downcase)}
      ...
      cityCode = (Code.all.find { |city| city["city_name"].gsub(/[^a-zA-Z]/, '').downcase == params[:trip][:trip_title].gsub(/[^a-zA-Z]/, '').downcase} || Code.all.find { |city| city["city_name"].gsub(/[^a-zA-Z]/, '').downcase.include? params[:trip][:trip_title].gsub(/[^a-zA-Z]/, '').downcase}).city_code
