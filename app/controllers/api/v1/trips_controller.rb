...
class Api::V1::TripsController < ApplicationController
  ...
  def create
    ...
    if Location.all.find {|location| location[:name_suffix].split(",")[0].downcase.include? (params[:trip][:trip_title].downcase.split(" ").join(""))}
      @locations = Location.all.find_all {|location| location[:name_suffix].split(",")[0].downcase.include? (params[:trip][:trip_title].downcase.split(" ").join(""))}
      render json: @locations
    else
      cityCode = Code.all.find { |city| city["city_name"].downcase.split(" ").join("") == params[:trip][:trip_title].downcase.split(" ").join("")}.city_code
